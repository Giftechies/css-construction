...
const Mobilenav = ({className}) => {
  ...
  return (
    <section>
      ...
      <div
        className={cn(
          " z-50 absolute inset-0 p-4 max-xl:flex justify-between hidden ",className,
          { invisible: sideopen }
        )}
      >
        <div className="logo w-[80px]">
          <Image src={'/img/logo/nav-logo.svg'} className=" w-full object-center object-cover " width={250} height={250} alt="logo"  />
        </div>
            ...
            <ul className="list text-white-1  mt-4 space-y-4 overflow-auto ">
              {mobilenav.map((item, idx) => (
                <li key={idx} className="border-b border-white-2 pb-2">
                  ...
                  >
                    {item.menuTitle}
                 {item.menuItems &&    <IconChevronDown
                  ...
                  {openIndex === idx && item.menuItems && (
                    <ul className="pl-4 space-y-4">
                      {item.menuItems.map((subItem, subIdx) => (
                        <li key={subIdx}>
                          <Link href={subItem.menuItemPath}>
                            {subItem.title}
                          </Link>
                        </li>
                      ))}
